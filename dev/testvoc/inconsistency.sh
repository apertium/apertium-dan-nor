...
if [[ $DIR == "dan-nob" ]]; then
lt-expand ../../apertium-dan-nor.dan.dix | grep -v '<prn><enc>' | grep -v 'REGEX' | grep -e ':<:' -e '\w:\w' | sed 's/:<:/%/g' | sed 's/:/%/g' | cut -f2 -d'%' |  sed 's/^/^/g' | sed 's/$/$ ^.<sent>$/g' | tee $TMPDIR/tmp_testvoc1.txt |
...
lt-expand ../../apertium-dan-nor.nob.dix | grep -v '<prn><enc>' | grep -v 'REGEX' | grep -e ':<:' -e '\w:\w' | sed 's/:<:/%/g' | sed 's/:/%/g' | cut -f2 -d'%' |  sed 's/^/^/g' | sed 's/$/$ ^.<sent><clb>$/g' | tee $TMPDIR/tmp_testvoc1.txt |
        apertium-pretransfer|
        apertium-transfer ../../apertium-dan-nor.nob-dan.t1x  ../../nob-dan.t1x.bin  ../../nob-dan.autobil.bin | tee $TMPDIR/tmp_testvoc2.txt |
...
lt-expand ../../apertium-dan-nor.nno.dix | grep -v '<prn><enc>' | grep -v 'REGEX' | grep -e ':<:' -e '\w:\w' | sed 's/:<:/%/g' | sed 's/:/%/g' | cut -f2 -d'%' |  sed 's/^/^/g' | sed 's/$/$ ^.<sent><clb>$/g' | tee $TMPDIR/tmp_testvoc1.txt |
        apertium-pretransfer|
        apertium-transfer ../../apertium-dan-nor.nno-dan.t1x  ../../nno-dan.t1x.bin  ../../nno-dan.autobil.bin | tee $TMPDIR/tmp_testvoc2.txt |
        lt-proc -d ../../nor-dan.autogen.bin  | sed 's/ \.//g' > $TMPDIR/tmp_testvoc3.txt
